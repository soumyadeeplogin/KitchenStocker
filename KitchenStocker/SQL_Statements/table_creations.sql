...
drop table item;
create table item (id int primary key  not null,  cat_id int, serial_no int, sub_cat_id int, quntity float(3,2), unit enum('kg','ltr','Pcs'), purchase_date datetime, update_date datetime, foreign key (cat_id) references category(id), foreign key (sub_cat_id) references sub_category(id));